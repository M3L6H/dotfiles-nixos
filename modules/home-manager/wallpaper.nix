...
{
  options = {
    wallpaper.awww.enable = mkEnableOption "enables awww wallpaper";
    wallpaper.mpvpaper.enable = mkEnableOption "enables mpvpaper wallpaper";
  ...
  config =
    let
      autoThemeScript = ''
        confFile="${dir}/''${selection%.*}.json"
        if ! [ -f "$confFile" ]; then
          matugen image "${dir}/$selection" --json hex -t scheme-smart --prefer less-saturation > "$confFile"
        fi
        matugen image "${dir}/$selection" -t scheme --prefer less-saturation --import-json "$confFile"
      '';
    in
    mkIf (config.wallpaper.awww.enable || config.wallpaper.mpvpaper.enable) {
      ...
      home = {
        file.".local/bin/choose-wallpaper.sh" = {
          ...
          text = ''
            ...
            selection="$(ls -I '*.json' ${dir} | rofi -dmenu)"
      ...
      wayland.windowManager.mango.settings.bind = mkIf config.mango.enable [
        "SUPER+SHIFT,W,spawn,/home/${username}/.local/bin/choose-wallpaper.sh"
      ];

      systemd.user = {
        services = {
          awww-daemon = mkIf config.wallpaper.awww.enable {
            Unit = {
          ...
          mpvpaper = mkIf config.wallpaper.mpvpaper.enable {
            Unit = {
        ...
        timers.mpvpaper-check = mkIf config.wallpaper.mpvpaper.enable {
          Unit = {
